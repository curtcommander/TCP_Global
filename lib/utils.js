...
function updateFiles(params) {
  return api('files', 'update', params);
}
...
async function getFileName(fileId) {
  const responseData = await getFiles({fileId, fields: 'name'});
...
async function _resolveFId(identifiers) {
  ...
  if (!parentId) return await getFileId(fName);
  return await getFileId(fName, {parentId});
}
