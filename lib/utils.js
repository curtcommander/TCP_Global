...
async function getFileId(fileName, paramsParent = {}) {
  ...
  if (responseData) {
    ...
    } else if (responseData.files.length === 0) {
...
async function getFileName(fileId) {
  let responseData;
  responseData = await this.getFiles({fileId, fields: 'name'});
...
async function _resolveFId(identifiers) {
  ...
  if (!parentId) return this.getFileId(fName, true);
  return this.getFileId(fName, {parentId}, true);
}
...
async function getMime(identifiers) {
  ...
  let responseData;
  responseData = await this.getFiles({
