...
async function _resolveParamsId(params) {
  ...
  // name given
    ...
    return await getFileId(params[propPrefix+'Name']);
...
async function getFileId(fileName, paramsParent = {}) {
  ...
  if (Object.keys(paramsParent).length) {
    const parentId = await _resolveParamsId(paramsParent);
...
function getFileIdPId(fileName, parentId) {
  if (!parentId) return getFileId(fileName);
...
function getFileIdPName(fileName, parentName) {
  if (!parentName) return getFileId(fileName);
...
async function _resolveFId(identifiers) {
  if (identifiers.fileId) return identifiersfileId;
  ...
  let parentId;
  if (Object.keys(pParams).length) parentId = await _resolveParamsId(pParams);
  if (!parentId) return await getFileIdPId(fName);
  return await getFileIdPId(fName, parentId);
}
...
async function getMime(identifiers) {
  const fileId = await _resolveFId(identifiers);
...
function getMimeFNamePId(fileName, parentId) {
  if (!parentId) return getMime({fileName});
...
function getMimeFNamePName(fileName, parentName) {
  if (!parentName) return getMime({fileName});
...
async function listChildren(identifiers, fields = 'files(name, id, mimeType)') {
  const folderId = await _resolveFId(identifiers);
...
function listChildrenFId(folderId, fields = 'files(name, id, mimeType)') {
  return listChildren({folderId}, fields);
}
...
function listChildrenFNamePId(
  ...
  if (!parentId) return listChildren({folderName}, fields);
...
function listChildrenFNamePName(
  ...
  if (!parentName) return listChildren({folderName}, fields);
