...
async function rename(identifiers, newName) {
    ...
    await utils.updateFiles(params);
}
...
async function mv(identifiers, newParentParams) {
    const fileId = await utils._resolveFId(identifiers);
    const responseData = await utils.getFiles({fileId, fields: 'parents'});
    const oldParentId = responseData.parents[0];
    const newParentId = await utils._resolveParamsId(newParentParams);
    const params = {
        fileId,
